...
const AIGuide: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div
        className={`fixed bottom-6 right-6 w-96 max-w-[90vw] bg-slate-900 rounded-2xl shadow-2xl border border-slate-700 z-50 flex flex-col transition-all duration-300 origin-bottom-right overflow-hidden ${
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-950">
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-slate-800 text-slate-400 px-4 py-3 rounded-2xl rounded-bl-none border border-slate-700 text-xs flex items-center gap-2">
        ...
        <div className="p-3 bg-slate-900 border-t border-slate-800">
          <div className="flex items-center gap-2 bg-slate-800 rounded-full px-4 py-2 border border-slate-700">
            <input
              ...
              className="flex-1 bg-transparent text-sm focus:outline-none text-slate-200 placeholder:text-slate-500"
              disabled={isLoading}
            />
            <button
              onClick={handleSend}
              disabled={isLoading || !input.trim()}
              className="text-amazon-primary hover:text-amazon-light disabled:opacity-50 transition-colors"
            >
              <Send size={18} />
            </button>
          </div>
